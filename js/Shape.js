define(['js/Line.js'],function(Line) {
  var Shape = function(points,fill,fillAlpha,border,borderColor,borderAlpha) {
    ...
    for(var i=1;i<points.length;i++) {
      ...
      this.lines.push(new Line(points[i-1].x,points[i-1].y,points[i].x,points[i].y));
    }
    this.graphics.lineTo(points[0].x,points[0].y);
    this.lines.push(new Line(points[points.length-1].x,points[points.length-1].y,points[0].x,points[0].y));
